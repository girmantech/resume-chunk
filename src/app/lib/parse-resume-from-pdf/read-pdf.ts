...
export const readPdf = async (fileUrl: string): Promise<TextItems> => {
  ...
  //this below logic makes sure that the text items are sorted in the order of their appearance in the pdf,
  //text in same line are gorouped together and are sorted left to right
  // textItems.sort((a, b) => {
  //   if (b.y === a.y) {
  //     return a.x - b.x;
  //   }
