...
import {
  ...
  hasOnlyLettersSpacesAmpersandsColons,
  isBold,
...
const isSectionLink = (text:string) => {
  return (
    SOCIALS.some((social) => text.includes(social))
  );
}
...
const isSectionTitle = (line: Line, lineNumber: number) => {
  ...
  if (
    textHasAtMost2Words &&
    hasOnlyLettersSpacesAmpersandsColons(textItem) &&
